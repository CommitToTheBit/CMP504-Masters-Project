...
You will first be asked to select your character (scrolling left/right to see all options available).
You may also click the 'Ellipses' button to run the simulation automatically.

On starting the simulation, it will run from 7.00pm-8.00pm (in storyworld). In this time, you can:
...
The seed for the interactive build only resets on closing the application; the 'Stop' button resets the simulation, but with the same seed.
Pressing 'Stop' therefore allows you to replay the same scenario, but from the perspective of a different character.

Use the mouse or arrow keys to navigate MiGS, left clicking to select choices.
...
The microstories, likelihoods data, etc., included in the experimental build are those used in the research questionnaire.
...
* Note that the interactive build does not generate likelihoods data; this has always been handled in a text console.
  This functionality can instead be tested in the experimental build.
  ...
  Other than this one change, the experimental build is as it was at the release of the questionnaire (and will include bugs later fixed by the interactive build).