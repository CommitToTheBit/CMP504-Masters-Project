...
public partial class Script
{
    ...
    private Dictionary<string, Vertex> LoadVertices(string path, List<string> characters)
    {
        ...
        Dictionary<string, JsonVertex>? jsonVertices = JsonSerializer.Deserialize<Dictionary<string, JsonVertex>>(File.ReadAllText(path));
        ...
        int briefings = (int)Math.Ceiling(0.33f * characters.Count);
    ...
    private Dictionary<string, Vertex> GenerateStartEdges(Dictionary<string, Vertex> vertices)
    {
        ...
        {
            /*if (!vertices["START"].IsAdjacent(source))
                vertices["START"].AddEdge(source, distanceFromStart[source]);

            foreach (string target in distanceFromStart.Keys.ToList().FindAll(x => !source.Equals(x)))
            {
                vertices[source].AddEdge(target, distanceFromStart[source] + distanceFromStart[target]);
            }*/
    ...
    {
        // MESSY 'SANITY CHECK' THAT THE SCRIPT IS LOADING CORRECTLY! //

        // STEP 1: Write top line...
        Console.Write("     ");
        foreach (string to in m_lines.Keys)
            Console.Write(to.Substring(0, Math.Min(to.Length, 4)).PadLeft(4) + " ");
        Console.WriteLine();

        // STEP 2: Write (utility-based) adjacency matrix...
        foreach (string from in m_lines.Keys)
        {
            Console.Write(from.Substring(0, Math.Min(from.Length, 4)).PadLeft(4) + " ");

            foreach (string to in m_lines.Keys)
            {
                if (m_lines[from].IsAdjacent(to))
                {
                    Console.Write(Math.Pow(0.5f, m_lines[from].GetEdge(to) - 1).ToString("0.00") + " ");
                    //Console.Write(m_lines[from].m_edges[to].ToString("0.") + " ");
                }
                else
                {
                    Console.Write("---- ");
