...
public class Stage
{
    ...
    public Stage(Dictionary<string, Characterisation> characterisations, int seed)
    {
    ...
    public Stage(Stage stage, int seed)
    {
        m_rng = new Random(seed);
    ...
    public void PrintCharactersAt(string focus = "NONFOCALISED")
    {
        ...
        foreach (string setting in m_settings.Keys)
        {
            ...
            Console.WriteLine("  " + setting + ((search) ? " (last searched at: " + m_settings[setting].m_characterLastLOS[focus] + ")" : "") + ":");
            foreach (string character in characters)
                Console.WriteLine("    " + character + " (last confirmed at: " + GetLastAt(character).Value + ((search) ? " => " + ((IsLost(focus, character)) ? "lost" : "not lost") : "") + ")");
            Console.WriteLine();
        }
    }

    public string PrintPhysicalState(string focus = "NONFOCALISED", bool printCharacters = false, bool print = true)
    {
        string text = "";
