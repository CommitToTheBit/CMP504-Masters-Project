...
public partial class Agent : Node
{
    public class Talk
    {
        ...
        {
            m_diegesis = ParseText(m_diegesis);
            m_mimesis = ParseText(m_mimesis);
        }

        private string ParseText(string text)
        {
            // FIXME: Build proper parser!
            while (true)
            {
                List<string> leftBrace = text.Split("{", 2).ToList();
                if (leftBrace.Count < 2)
                    break;

                List<string> rightBrace = leftBrace.Last().Split("}", 2).ToList();
                if (rightBrace.Count < 2)
                    break;

                string identification = (m_accusation.ContainsKey(rightBrace.First())) ? m_accusation[rightBrace.First()] : "*someone*";
                text = leftBrace.First() + identification + rightBrace.Last();
            }
    ...
    public List<Talk> GetTalkChoices(int tick)
    {
        ...
        while (talks.Count < 3 && edges.Count > 0)
        {
        ...
        {
            talks[i].ParseLine();
            talks[i].SetUtility(WaitingUtility(talks[i]) * EdgeUtility(talks[i])); 
        }       

        return talks.FindAll(x => x.IsNontrivial());
    }
    
    public Talk GetTalk(int tick)
    {
        List<Talk> choice = GetTalkChoices(tick);
        return (choice.Count > 0) ? choice.First() : new Talk();
    }
    ...
    private double RepetitionUtility(Talk talk)
    {
        ...
        double repetitivity = (double)m_script.GetTotalAwarenesses(talk.m_line, talk.m_internalAudience).Select(x => Math.Pow(x.Value, 5.0)).Sum() / talk.m_internalAudience.Count;

        return UtilityCurves.ExponentialAsymptote(0.2, repetitivity, 1.0, 0.0);
    }
    ...
    private double AmbiguityUtility(Talk talk)
    {
        ...
        return UtilityCurves.ExponentialAsymptote(0.99, ambiguities, 1.0, 0.5);
    }
    ...
    private double DirectnessUtility(Talk talk)
    {
        ...
        double directs = talk.m_internalAudience.Intersect(talk.m_accusation.Values).ToList().FindAll(x => !m_characterisation.GetVendettas(true, false).Contains(x)).Count();

        return (directs > 0.0) ? 0.05 : 1.0;// UtilityCurves.ExponentialAsymptote(0.2, directs, 1.0, 0.01);
    }
