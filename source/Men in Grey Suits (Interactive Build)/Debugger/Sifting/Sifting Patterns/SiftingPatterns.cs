...
public partial class SiftingPatterns : Node
{
    ...
    {
        string path = (OS.IsDebugBuild()) ? ProjectSettings.GlobalizePath("res://Assets/Narrative/sifting_patterns.json") : OS.GetExecutablePath().GetBaseDir() + "/Assets/Narrative/sifting_patterns.json";
        m_patterns = LoadPatterns(path);
    }
    ...
    private Dictionary<string, Microstory> LoadPatterns(string path)
    {
        ...
        Dictionary<string, JsonMicrostory>? jsonPatterns = JsonSerializer.Deserialize<Dictionary<string, JsonMicrostory>>(System.IO.File.ReadAllText(path))!;
    ...
    {
        // NB: This dictionary is referred to as "matches" in the dissertation...
        ...
        foreach (string focalisation in traces.Keys)
        {
            ...
            {
                // FIXME: Yes, I know - this is hard-coded! We could search for all sifting pattern here...
                // ...But since DialogueDebugger.cs only surfaces 'He Said, She Said', why slow the executable down?
                if (!pattern.Equals("He Said, She Said"))
                    continue;
