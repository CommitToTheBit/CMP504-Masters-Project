...
public partial class Microstory : Node
{
    ...
    public Microstory()
    {
        ...
        m_attributeConstraints = new Dictionary<string, List<string>>();
        m_valueConstraints = new Dictionary<string, List<KeyValuePair<string, string>>>();
        m_relations = new List<List<string>>();
    }
    ...
    public string PrintMicrostory()
    {
        ...
        {
            text += ((!focalisation.Equals("NONFOCALISED")) ? focalisation + " recalls...\n\n": string.Empty);
            for (int i = 0; i < slots[focalisation].Count; i++)
            {
                string slot = slots[focalisation][i];

                string prefix = "In ";//(!focalisation.Equals("NONFOCALISED") && i > 0) ? "Later, in " : "In ";
                bool preamble = true;

                bool refreshSetting = true;

                if (!focalisation.Equals("NONFOCALISED") && i > 0)
                {
                    string lastSlot = slots[focalisation][i-1];

                    refreshSetting = !m_beatSlots[slot].GetFirstValue("initial setting").Equals(m_beatSlots[lastSlot].GetFirstValue("initial setting"));

                    if (m_beatSlots[slot].GetTick() == m_beatSlots[lastSlot].GetTick())
                    {    
                        prefix = "At the same time" + ((refreshSetting) ? ", in " : string.Empty);
                    }
                    else if (m_beatSlots[slot].GetTick() == m_beatSlots[lastSlot].GetTick() + 1)
                    {
                        prefix = "Then" + ((refreshSetting) ? ", in " : string.Empty);
                        preamble = refreshSetting;
                    }
                    else 
                    {
                        prefix = "Later, in "; //+ ((refreshSetting) ? "still " : string.Empty) + "in ";
                        refreshSetting = true;
                    }    

                }

                text += m_beatSlots[slot].PrintBeat(prefix, preamble, refreshSetting) + ((i < slots[focalisation].Count - 1) ? "\n\n" : string.Empty);
            }
        }
